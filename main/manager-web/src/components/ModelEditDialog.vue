...
export default {
  ...
  data() {
    return {
      ...
      fieldJsonMap: {}, // Used to store JSON fields in string form
      sensitive_keys: [
        "api_key",
      ...
      originalValues: {}, // Store original values for restoration on blur
      form: {
        id: "",
        modelType: "",
        modelCode: "",
  ...
  methods: {
    ...
    validateJson(value) {
      try {
        ...
        this.$message.error({
          message: 'Must input dictionary format (e.g. {"key":"value"}), original data will be used on save',
          showClose: true,
        });
        return null;
      } catch (e) {
    ...
    isSensitiveField(fieldName) {
      ...
      // Exact match with 7 sensitive words defined in keyMap
      return this.sensitive_keys.includes(lowerFieldName);
    },

    // Get English name corresponding to sensitive field
    ...
    handleInputFocus(field, value) {
      // If value contains asterisks, clear display
      if (value && value.includes("*")) {
        // Store original value for restoration on blur
    ...
    handleInputBlur(field) {
      ...
      if (this.isSensitiveField(field)) {
        // If value is empty, restore masked value
        if (!this.form.configJson[field] || this.form.configJson[field].trim() === "") {
          // If there's an original value, restore it; otherwise set to mask prompt
    ...
    handleJsonInputBlur(field) {
      // JSON fields don't need special handling as they usually don't contain simple sensitive information
    },
  },
};
</script>
