...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("fetchPubConfig").then(() => {
      ...
      // After pub-config API call completes, re-initialize featureManager to ensure using latest configuration
      featureManager.waitForInitialization().then(() => {
        console.log('featureManager re-initialization completed, using pub-config configuration');
      }).catch(error => {
        console.warn('featureManager re-initialization failed:', error);
      });
    });
  },
  methods: {
    ...
    async login() {
      ...
      Api.user.login(
        ...
        (err) => {
          // Directly use internationalized message returned from backend
