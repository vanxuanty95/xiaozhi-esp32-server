              ...
              <div class="form-content">
                <div class="form-grid">
                  <div class="form-column">
                    ...
                    <el-form-item label="Context Sources:" class="context-provider-item">
                      <div style="display: flex; align-items: center; justify-content: space-between;">
                        <span style="color: #606266; font-size: 13px;">
                          Successfully added {{ currentContextProviders.length }} source(s). <a href="https://github.com/xinnan-tech/xiaozhi-esp32-server/blob/main/docs/context-provider-integration.md" target="_blank" class="doc-link">How to deploy context sources</a>
                        </span>
                        <el-button
...
export default {
  ...
  data() {
    return {
      ...
      voiceOptions: [],
      voiceDetails: {}, // Save complete voice information
  ...
  methods: {
    ...
    fetchAgentConfig(agentId) {
      Api.agent.getDeviceConfig(agentId, ({ data }) => {
        if (data.code === 0) {
          ...
          // First ensure allFunctions is loaded (if not, fetchAllFunctions first)
          const ensureFuncs = this.allFunctions.length
            ? Promise.resolve()
            : this.fetchAllFunctions();

          ensureFuncs.then(() => {
            // Merge: Complete full metadata information according to pluginId (id field)
            ...
            // Backup original for restoration on cancel
        ...
        if (model.type != "LLM") {
          Api.model.getModelNames(model.type, "", ({ data }) => {
            if (data.code === 0) {
              ...
              // If it's an intent recognition option, need to update visibility based on current LLM type
      ...
      Api.model.getModelVoices(modelId, "", ({ data }) => {
        if (data.code === 0 && data.data) {
          this.voiceOptions = data.data.map((voice) => ({
            value: voice.id,
            label: voice.name,
            // Copy audio-related fields to ensure hasAudioPreview can detect
            ...
            // New: Add clone audio related fields
            cloneAudioUrl: voice.cloneAudioUrl,
            hasCloneAudio: voice.hasCloneAudio || false,
            // Save training status field for determining if it's clone audio
            train_status: voice.trainStatus,
          }));
          // Save complete voice information, add debug information
          console.log("Voice data retrieved:", data.data);
    ...
    handleModelChange(type, value) {
      ...
      if (type === "Memory") {
        if (value === "Memory_nomem") {
          ...
          // Models with memory function, default to recording text and voice
    ...
    hasAudioPreview(item) {
      // Check if item contains valid audio URL field or clone audio field
      // Clone audio is determined by hasCloneAudio flag or ID format (ID not starting with TTS)
      const isCloneAudio =
      ...
      const audioFields = [
        ...
        item.sample_voice,
        item.referenceAudio,
        item.cloneAudioUrl, // Clone audio URL
      ];

      // Check if any audio field is a valid URL
      ...
      if (this.playingVoice && this.currentPlayingVoiceId === voiceId) {
        if (this.isPaused) {
          ...
          this.currentAudio.play().catch((error) => {
            ...
            this.$message.warning("Unable to resume audio playback");
          });
          this.isPaused = false;
        } else {
          // Pause playback
    ...
    playVoicePreview(voiceId = null) {
      ...
      if (!targetVoiceId) {
        this.$message.warning("Please select a voice first");
        return;
      }
      ...
      try {
        ...
        if (voiceDetail) {
          // First check if it's a clone audio (judge by ID format, IDs not starting with TTS)
          isCloneAudio =
            voiceDetail.hasCloneAudio ||
            (voiceDetail.id && !voiceDetail.id.startsWith("TTS"));
          console.log(
            "Clone audio judgment result:",
            isCloneAudio,
            "Training status:",
          ...
          if (isCloneAudio && voiceDetail.id) {
            // For clone audio, use the correct interface provided by backend
            // Note: Need to get audio URL in two steps here
            // 1. First get audio download ID
            // 2. Then use this ID to build playback URL
            // Due to async operations, we need to request getAudioId first
            console.log("Detected clone audio, preparing to get audio URL:", voiceDetail.id);

            // Create a Promise to handle async audio URL retrieval
            const getCloneAudioUrl = () => {
              return new Promise((resolve) => {
                // First call getAudioId interface to get temporary UUID
                RequestService.sendRequest()
                  .url(`${getServiceUrl()}/voiceClone/audio/${voiceDetail.id}`)
                  .method("POST")
                  .success((res) => {
                    if (res.data.code === 0 && res.data.data) {
                      // Process returned data format, wrap another layer of .data on top of res.data
                      const audioId = res.data.data;
                      console.log("Retrieved audio ID:", audioId);
                      // Use returned UUID to build playback URL
                      const playUrl = `${getServiceUrl()}/voiceClone/play/${audioId}`;
                      console.log("Built clone audio playback URL:", playUrl);
                      resolve(playUrl);
                    } else {
                  ...
                  .networkFail((err) => {
                    console.error("Failed to request audio ID interface:", err);
                    resolve(null);
                  })
                  .send();
              });
            ...
            const timeoutId = setTimeout(() => {
              if (this.currentAudio && this.playingVoice) {
                this.$message.warning("Audio loading time is long, please retry later");
            ...
            this.currentAudio.onerror = () => {
              ...
              this.$message.warning("Clone audio playback failed");
              this.playingVoice = false;
            };
            ...
            getCloneAudioUrl().then((url) => {
              if (url) {
                ...
                this.currentAudio.play().catch((error) => {
                  ...
                  this.$message.warning("Unable to play clone audio");
                  this.playingVoice = false;
                });
              } else {
                clearTimeout(timeoutId);
                this.$message.warning("Failed to get clone audio");
                this.playingVoice = false;
              }
            });

            // Return to avoid continuing with normal audio playback logic below
            return;
          } else {
            // For normal audio, try various possible URL fields
            audioUrl =
              ...
              voiceDetail.cloneAudioUrl; // Clone audio URL
          }

          // If not found, try checking if there are URL format fields
          if (!audioUrl) {
        ...
        if (!audioUrl) {
          ...
          this.$message.warning("This voice has no preview audio available");
          return;
        }
        ...
        if (!isCloneAudio) {
          ...
          this.currentAudio.onerror = () => {
            ...
            this.$message.warning("Audio playback failed");
            this.playingVoice = false;
          };
          ...
          this.currentAudio.play().catch((error) => {
            ...
            this.$message.warning("Unable to play audio");
            this.playingVoice = false;
          });
        }
      } catch (error) {
        console.error("Error during audio playback:", error);
        this.$message.error("Error during audio playback");
        this.playingVoice = false;
      }
    },
