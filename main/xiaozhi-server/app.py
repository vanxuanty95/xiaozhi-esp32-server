...
async def wait_for_exit() -> None:
    ...
    if sys.platform != "win32":  # Unix / macOS
        ...
        # let KeyboardInterrupt bubble up to asyncio.run to eliminate process exit blocking caused by leftover regular threads
        try:
...
async def main():
    ...
    logger.bind(tag=TAG).info(
        "=======The above addresses are websocket protocol addresses, please do not access with a browser======="
    )
    logger.bind(tag=TAG).info(
