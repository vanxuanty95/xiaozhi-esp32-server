...
async def handleAudioMessage(conn, audio):
    ...
    if hasattr(conn, "just_woken_up") and conn.just_woken_up:
        ...
        return
    # In manual mode, don't interrupt currently playing content
    if have_voice:
        if conn.client_is_speaking and conn.client_listen_mode != "manual":
            await handleAbortMessage(conn)
    # Device idle detection for extended periods, used for say goodbye
...
async def startToChat(conn, text):
    ...
    try:
        ...
        if text.strip().startswith("{") and text.strip().endswith("}"):
            ...
            if "speaker" in data and "content" in data:
                ...
                # Use JSON format text directly, don't parse
                actual_text = text
    ...
    if intent_handled:
        # If intent has been handled, don't continue chatting
        return
    ...
    if conn.last_activity_time > 0.0:
        ...
        if (
            ...
            if end_prompt and end_prompt.get("enable", True) is False:
                conn.logger.bind(tag=TAG).info("Ending conversation, no need to send end prompt")
                await conn.close()
                return
            prompt = end_prompt.get("prompt")
            if not prompt:
                prompt = "Please end this conversation with emotional and reluctant words, starting with 'Time flies so fast'."
...
async def max_out_size(conn):
    ...
    conn.client_abort = False
    text = "Sorry, I'm a bit busy right now. Let's chat again at this time tomorrow, agreed? See you tomorrow, bye!"
...
async def check_bind_device(conn):
    if conn.bind_code:
        ...
        text = f"Please log in to the control panel, enter {conn.bind_code} to bind the device."
        await send_stt_message(conn, text)
        ...
        # Play unbound prompt
        conn.client_abort = False
        text = f"Version information for this device was not found. Please correctly configure the OTA address and recompile the firmware."
        await send_stt_message(conn, text)
