...
2. Reporting thread lifecycle is bound to the connection object
...
def opus_to_wav(conn, opus_data):
    ...
    Args:
        output_dir: Output directory (reserved parameter for interface compatibility)
        opus_data: Opus audio data
...
def enqueue_tts_report(conn, text, opus_data):
    ...
    try:
        ...
        if conn.chat_history_conf == 2:
            ...
            conn.logger.bind(tag=TAG).debug(
                f"TTS data added to reporting queue: {conn.device_id}, not reporting audio"
            )
    except Exception as e:
...
def enqueue_asr_report(conn, text, opus_data):
    ...
    try:
        ...
        if conn.chat_history_conf == 2:
            ...
            conn.logger.bind(tag=TAG).debug(
                f"ASR data added to reporting queue: {conn.device_id}, not reporting audio"
            )
    except Exception as e:
