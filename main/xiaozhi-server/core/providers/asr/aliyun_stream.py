...
class ASRProvider(ASRProviderBase):
        ...
        try:
            while self.asr_ws and not conn.stop_event.is_set():
                try:
                    ...
                    if status != 20000000:
                        if status in [40000004, 40010004]:  # Connection timeout or client disconnected
    ...
    async def _cleanup(self, conn):
        ...
        # Determine if termination request needs to be sent
        ...
        if self.asr_ws and should_stop:
            try:
                ...
                logger.bind(tag=TAG).debug("Sending ASR termination request")
                await self.asr_ws.send(json.dumps(stop_msg, ensure_ascii=False))
                await asyncio.sleep(0.1)
                logger.bind(tag=TAG).debug("ASR termination request sent")
            except Exception as e:
                logger.bind(tag=TAG).error(f"Failed to send ASR termination request: {e}")
        
        # Reset status (after termination request is sent)
        self.is_processing = False
