            ...
            if msg_id == 1:  # mcpInitializeID
                ...
                if (
                    ...
                    if next_cursor:
                        ...
                        logger.bind(tag=TAG).info(
                            f"MCP endpoint tool retrieval completed, total {len(mcp_client.tools)} tools"
                        )
                else:
                    logger.bind(tag=TAG).warning(
                        "MCP endpoint tool list response result is empty or format error"
                    )
                return
...
async def send_mcp_endpoint_tools_list(mcp_client: MCPEndpointClient):
    """Send MCP endpoint tool list request"""
    payload = {
        "jsonrpc": "2.0",
        "id": 2,  # mcpToolsListID
        "method": "tools/list",
