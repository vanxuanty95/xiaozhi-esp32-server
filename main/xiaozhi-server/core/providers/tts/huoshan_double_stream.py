...
EVENT_ConnectionFailed = 51  # Connection failed (may be unable to pass permission authentication)
...
class TTSProvider(TTSProviderBase):
    ...
    async def _ensure_connection(self):
        """Establish new WebSocket connection and start monitor task (only first time)"""
        try:
            if self.ws:
            ...
            # After connection is established, start monitor task
    ...
    async def start_session(self, session_id):
        ...
        try:       
            # Wait for previous session to end, wait at most 3 times
            for _ in range(3):
    ...
    async def close(self):
        ...
        if self._monitor_task:
            try:
                ...
                logger.bind(tag=TAG).warning(f"Error canceling monitor task when closing: {e}")
        ...
        try:
            while not self.conn.stop_event.is_set():
                try:
                    ...
                    # Only process responses for current active session
                    if res.optional.sessionId and self.conn.sentence_id != res.optional.sessionId:
                        # If it's a session end related event, reset state even if session ID doesn't match
                    ...
                    if res.optional.event == EVENT_SessionCanceled:
                        logger.bind(tag=TAG).debug(f"Server resources released successfully～～")
                        self.activate_session = False
    ...
    # Read string content from a segment of res array
