...
def check_ffmpeg_installed() -> bool:
    ...
    Returns:
        bool: Returns True if ffmpeg is normally available; otherwise raises ValueError exception.

    Raises:
        ValueError: Raises detailed prompt information when ffmpeg is not installed or dependencies are missing.
    """
    try:
        ...
        # If version information is not detected, also treat as abnormal
        ...
        if "libiconv.so.2" in stderr_output:
            ...
            error_msg.append("⚠️ System did not find ffmpeg executable file.")
            error_msg.append("Solution: Execute in the current conda environment:")
...
def opus_datas_to_wav_bytes(opus_datas, sample_rate=16000, channels=1):
    ...
    try:
        ...
        for opus_frame in opus_datas:
            # Decode to PCM (returns bytes, 2 bytes per sample)
...
def is_valid_image_file(file_data: bytes) -> bool:
    ...
    Returns:
        bool: Returns True if it's a valid image format, otherwise returns False
    """
...
def validate_mcp_endpoint(mcp_endpoint: str) -> bool:
    ...
    # 2. Check if it contains key, call keywords
    if "key" in mcp_endpoint.lower() or "call" in mcp_endpoint.lower():
        return False

    # 3. Check if it contains /mcp/ keyword
