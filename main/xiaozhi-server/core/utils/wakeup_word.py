...
class WakeupWordsConfig:
    ...
    def _load_config(self) -> Dict:
        """Load configuration file, use cache mechanism"""
    ...
    def _save_config(self, config: Dict):
        """Save configuration to file, use file lock protection"""
        try:
    ...
    def update_wakeup_response(self, voice: str, file_path: str, text: str):
        """Update wakeup word reply configuration"""
        try:
            # Filter emoji
            ...
            print(f"Failed to update wakeup word reply configuration: {e}")
            raise

    def generate_file_path(self, voice: str) -> str:
        """Generate audio file path, use voice hash value as filename"""
        try:
            # Generate voice hash value
            voice_hash = hashlib.md5(voice.encode()).hexdigest()
